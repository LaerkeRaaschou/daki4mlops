...
def _setup_case():
    ...
    return model, loader, criterion, optimizer, device, epoch, device_type, local_rank


def test_train_model_returns_finite_loss(monkeypatch):
    monkeypatch.setattr(train.wandb, "log", lambda payload: None)

    model, loader, criterion, optimizer, device, epoch, device_type, local_rank = _setup_case()

    avg_loss = train.train_model(model, criterion, loader, optimizer, device, epoch, device_type, local_rank)
...
def test_train_model_updates_weights(monkeypatch):
    ...
    _ = train.train_model(model, criterion, loader, optimizer, device, epoch, device_type, local_rank)
