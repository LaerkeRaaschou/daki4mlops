...
def test_train_model_returns_finite_loss(monkeypatch):
    ...
    model, loader, criterion, optimizer, device = _setup_case()

    avg_loss = train.train_model(model, criterion, loader, optimizer, device, epoch=1)
...
def test_train_model_updates_weights(monkeypatch):
    ...
    _ = train.train_model(model, criterion, loader, optimizer, device, epoch=1)
