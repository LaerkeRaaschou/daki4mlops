...
def test_train_model_returns_finite_loss(monkeypatch):
    ...
    model, loader, criterion, optimizer, device, epoch, device_type, local_rank = (
        _setup_case()
    )

    avg_loss = train.train_model(
        model, criterion, loader, optimizer, device, epoch, device_type, local_rank
    )
